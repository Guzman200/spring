...
function llenarSelects (){
    ...
    $.ajax({
        "url" : "select_grafica.do",
        dataType: 'json',
        success: function (data, textStatus, jqXHR) {
            let selectGrafica = document.getElementById("selectGrafica");
            for( let item of data.data ){
                let option = document.createElement("option");
                option.text = item.type;
                option.value = item.id_tg;
                selectGrafica.appendChild(option);     
            }  
        },
        error: function (jqXHR, textStatus, errorThrown) {
            console.log(textStatus);
...
$(document).on('click', '.btnEditar', function () {
    ...
    $("#selectGrafica").val(data['id_tg']);
    $("#serieX").val(data['seriex']);
    $("#serieY").val(data['seriey']);
...
formAddControl.addEventListener('submit', (e) => {
    ...
    if(ban){
       if(option === 1){
           url = "insert_controlTres.do";
           id_g = 0;
           id_e = 0;
           id_p = 0;
       }else if(option === 2){
           url = "update_controlTres.do";
       }
...
function  validarSelects(){
    ...
    if(selectModulo.value === "default"){
        ...
        return false;
    }else if(selectGrafica.value === "default"){
        notificarError("Selecciona un tipo de grafica");
        return false;
